...
{
	if (event & 0x20)  // si fin d’émission...
	{
	...
	while (1) 
		{
			...
			if(resultat == 0) data_buf[0] = 0x00; // data à envoyer à placer dans un tableau de char
			else data_buf[0] = 0xFF;
...
{			
		while(1)
		{				
			Driver_USART2.Receive(tabIA,1); // tableau de 1 case
			osSignalWait(0x05, osWaitForever);
			osSignalSet(id_CANthreadT_IA,0x01);	
			if(tabIA[0] == 0x0f)haut_parleurs(PANNEAU_130); // panneau 130
			if(tabIA[0] == 0x0e)haut_parleurs(PANNEAU_50); // panneau 50
			if(tabIA[0] == 0x10)haut_parleurs(PANNEAU_INTERDICTION);	// panneau interdit 
			if(tabIA[0] == 0x07)haut_parleurs(FEU_ROUGE);  // feu rouge
			if(tabIA[0] == 0x11)haut_parleurs(PANNEAU_STOP); //panneau stop
			GLCD_DrawString(1,1,"OK");
		}
}
	...
	while (1) 
		{
			...
			if(tabIA[0] == 0x0e) data_buf[0] = 0x0e; // data à envoyer à placer dans un tableau de char
			if(tabIA[0] == 0x0f) data_buf[0] = 0x0f;
			if(tabIA[0] == 0x10) data_buf[0] = 0x10;
			if(tabIA[0] == 0x07) data_buf[0] = 0x07;
			if(tabIA[0] == 0x11) data_buf[0] = 0x11;
